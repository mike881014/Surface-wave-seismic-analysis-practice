function wigx (Data,t,x,scal,linecolor,fillcolor,amx)
%wigx    wigx (Data,t,x,scal,linecolor,fillcolor,amx) to plot seismic data in wiggle format.
...
%---------------------------------------------------------------------------------
   if (nargin > 5)
      ...
      npos = length(i1);
		...
		patch( aa+x(i) , zzz,  fillcolor); % patch with fillcolor
	end	
%----------------------------------------------------------------------------------
	...
	line( 'Color',linecolor,'EraseMode','background',  ...
	 'LineWidth',linewidth, ...
	 'Xdata', tr+x(i), 'Ydata',t);	% negatives line
